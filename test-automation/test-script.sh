...
BACKEND_AVAILABILITY="0.44"
# BACKEND_AVAILABILITY=$(curl --silent "${PROMETHEUS_QUERY_URL}$( urlencode "${BACKEND_MUT_QUERY}" )" | jq .data.result[0].value[1])
# echo "Backend availability: ${BACKEND_AVAILABILITY}"
BACKEND_MUT="132"
# BACKEND_MUT=$(curl --silent "${PROMETHEUS_QUERY_URL}$( urlencode "${BACKEND_AVAILABILITY_QUERY}" )" | jq .data.result[0].value[1])
