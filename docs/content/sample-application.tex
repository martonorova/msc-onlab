...
All the above mentioned components are packaged with Docker and deployed to a Kubernetes cluster. In order to achieve that, various Kubernetes definitions are needed that are fairly similar in case of each component. In this section only the backend related configurations and the definition of the Horizontal Pod Autoscaler are presented ot avoid unnecessary repetitions. However, all the definitions can be sound in the thesis repository \cite{ThesisRepo} under \texttt{helm/kubedepend/templates}.
...
Deploying the backend component requires three type of Kubernetes definitions. First, a \texttt{ConfigMap} is used to store the configuration of the backend. A \texttt{Deployment} object is necessary to create a \texttt{Pod} that runs the backend in a container in a somewhat fault tolerant way using a \texttt{ReplicaSet}. Lastly, a \texttt{Service} is needed to create an unchanging address for the backend inside the cluster so other components can communicate with it.
...
\begin{minipage}{\linewidth}
	\begin{lstlisting}[caption={Backend \texttt{ConfigMap}}]
	apiVersion: v1
	kind: ConfigMap
	metadata:
		name: backend-config
